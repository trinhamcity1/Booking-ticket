...
const DirectInfo = (props) => {
    const {flight_leg, setFlightLeg} = useContext(FlightsContext);
    ...
    useEffect(() =>{
        const fetchData = async() => {
            const response = await FlightFinder.get(`/${flight_id}/direct`);
            console.log(response.data.data);
            // SetSeatsAvailable(response.data.data.flight.seats_available);

            SetLegNo(response.data.data.flight_leg.leg_no);

            SetDepartureAirport0(response.data.data.flight_leg.departure_airport);
            SetArrivalAirport0(response.data.data.flight_leg.arrival_airport);


            SetScheduledDeparture0(response.data.data.flight_leg.scheduled_departure);
            SetScheduledArrival0(response.data.data.flight_leg.scheduled_arrival);
    ...
    return (
        ...
        <div>
        <h1>This flight is a <b>DIRECT</b> flight</h1>
            ...
            <form action="">
                ...
                <div className ="form-group">
                    ...
                    <input disabled value={departure_airport0} onChange={e => SetDepartureAirport0(e.target.value)} id="departure_airport" className="form-control" type="text"/>
                </div>

                <div className ="form-group">
                    <label htmlFor="arrival_airport1">Arrival airport</label>
                    <input disabled value={arrival_airport0} onChange={e => SetArrivalAirport0(e.target.value)} id="arrival_airport" className="form-control" type="text"/>
                </div>
